do
    ...
    do
        ...
        Input.defaultStyle = {
            ...
            font = {serif = "sansserif", style = "r", size = 10}
        }
        ...
        function Input:draw(gc, x, y, width, height, isColor)
            ...
            if self.hasFocus and value ~= "" then
                gc:fillRect(x + (text == value and strWidth + 2 or width - 4),
                            y, 1, height)
            end
        ...
        function Input:charIn(char)
            ...
            if self.number then newValue = tonumber(newValue) end

            if self.disabled or not newValue then return end

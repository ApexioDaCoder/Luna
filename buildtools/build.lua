...
function assert(bool, errormsg)
    if not bool then error(errormsg or "", 0) end
    return bool
end
...
function processFile(filename, path)
    luafiles = luafiles + 1
    assert(luafiles <= MAX_FILES,
           "Too many files to include! Are you sure you don't have an include loop?")

    local out = ""

    local file = assert(io.open(filename, "r"),
                        filename .. " could not be read! Aborting.")
    ...
    out = filecontent:gsub("%-%-include \"([^\"]*)\"", function(f)
        local f = path .. f
        return processFile(f, getPath(f))
    end)
...
local tmpfile = assert(io.open(tmpname, "w"),
                       "Failed to create temp file. Are you sure you have permissions to the current directory ?")
tmpfile:write(luaout)
tmpfile:close()

print("Trying to build project with Luna...")
assert(os.execute("./luna " .. tmpname .. " " .. outfilename),
       "Building with Luna failed! You can try to build " .. tmpname ..
           " manually")
